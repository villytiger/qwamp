...
#include "qwamprawsession.h"
#include "qwamprawsession_p.h"
...
enum FrameType { kRegularFrameType = 1, kPingFrameType = 2, kPongFrameType = 3, kUnknownFrameType = 4 };
...
QWAMP_BEGIN_NAMESPACE

SessionPrivate::SessionPrivate(QIODevice* io) : mIo(io), mState(kNewSessionState), mFrameLength(0), mRequestId(0) {
	if (mIo->openMode() & QIODevice::Unbuffered) throw "todo";
}

void SessionPrivate::start() {
	const quint8 buffer[] = {kHandshakeMagic, (kMaxInFrameSizeExponent << 4) + kJsonSerializer, 0x00, 0x00};
...
void SessionPrivate::sendMessage(const QJsonArray& data) { sendFrame(kRegularFrameType, QJsonDocument(data).toJson()); }
...
void SessionPrivate::sendFrame(int frameType, const QByteArray& data) {
	...
	const quint8 buffer[] = {static_cast<quint8>(frameType), static_cast<quint8>(size >> 16),
...
void SessionPrivate::processResult(const QJsonArray& data) {
	...
	QVariantList arguments = (data.size() < 4) ? QVariantList() : data[3].toArray().toVariantList();

	QVariantMap argumentsKw = (data.size() < 5) ? QVariantMap() : data[4].toObject().toVariantMap();
...
void SessionPrivate::processError(const QJsonArray& data) {
	...
	QVariantList arguments = (data.size() < 6) ? QVariantList() : data[5].toArray().toVariantList();

	QVariantMap argumentsKw = (data.size() < 7) ? QVariantMap() : data[6].toObject().toVariantMap();
...
void SessionPrivate::onMessage(const QJsonArray& data) {
	switch (data[0].toInt()) {
	case kResultMessageCode:
		return processResult(data);
	case kErrorMessageCode:
		return processError(data);
	case kGoodbyeMessageCode:
		return processGoodbye(data);
	default:
		throw "todo";
	}
}
...
void SessionPrivate::onMessageReceived(const QByteArray& data) {
	...
	switch (mState) {
	case kEstablishedSessionState:
		return onMessage(arr);
	case kHelloSentSessionState:
		return onHelloReply(arr);
	case kGoodbyeSentSessionState:
		return onGoodbyeReply(arr);
	default:
		throw "todo";
	}
}

void SessionPrivate::onPingReceived(const QByteArray& data) { sendFrame(kPongFrameType, data); }

void SessionPrivate::onPongReceived(const QByteArray& data) { (void)data; }
...
void SessionPrivate::onDataReceived() {
	...
	switch (mFrameType) {
	case kRegularFrameType:
		return onMessageReceived(buffer);
	case kPingFrameType:
		return onPingReceived(buffer);
	case kPongFrameType:
		return onPongReceived(buffer);
	default:
		Q_ASSERT(!"Unknown message type");
	}
}

Session::Session(QIODevice* io, QObject* parent) : QObject(parent), d_ptr(new SessionPrivate(io)) {
	d_ptr->q_ptr = this;
}
